...
\begin{center}
	\begin{tabularx}{0.9\textwidth}{lllX}
		...
		\code{DataSaveMode} & str                  & \code{disp\_only}& Amount of data saved to the results file. \\
		& & & \code{disp\_only} -- saves only the 7 essential variables (coordinates, displacements, CZNSSD, etc.) to reduce file size. \\
		& & & \code{All} -- saves all 17 internal computation variables (useful for debugging). \\
		\code{DataCompression} & bool               & \code{True}   & Enables zlib compression for the results file, reducing disk space requirements. \\
