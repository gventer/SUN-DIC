...
\begin{center}
	\begin{tabularx}{0.9\textwidth}{lllX}
		...
		\code{DataSaveMode} & str                  & \code{DispOnly}& Data saved to the results file. \\
		& & & \code{DispOnly} -- saves only the essential variables required for post-processing \\
		& & & \code{All} -- saves all internal variables (useful for advanced users or specialized debugging) \\
		\code{DataCompression} & bool               & \code{True}   & Enables \code{zlib} compression for the results file \\
...
\begin{center}
	\begin{tabularx}{0.9\textwidth}{lllX}
		...
		\code{InterpolationOrder}    & int   & \code{3}      & \code{3} -- cubic interpolation (more stable, usually sufficient) \\
