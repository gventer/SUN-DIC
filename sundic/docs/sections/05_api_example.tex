...
\subsection{Time History Plots}
When the analysis contains more than one image pair (thus more than two images), time history plots can be created to investigate how a displacement or strain component develops over time for one more points in the ROI.  The \code{plotDispTimeHistory} and \code{plotStrainTimeHistory} functions can be used to create these plots.  The listing below shows how to generate time history graphs for the displacement magnitude for points located at (300,300), (800,300) and (900,300) pixels.  This results in a 2D graph with three lines.  The second function call in the listing generates a similar plot for the Von Mises strain for the same three points, also resulting in a graph with three lines.  These functions will use all image pairs available in the results file.

\begin{lstlisting}
import sundic.post_process as sdpp

# Setup the list of points to track
points=[(300,300), (800,300), (900,300)]

# Create the displacement time history graph
sdpp.plotDispTimeHistory('results.sdic', points, 
	dispComp=sdpp.DispComp.DISP_MAG)

# Create the strain time history graph
sdpp.plotStrainTimeHistory('results.sdic', points, 
	strainComp=sdpp.StrainComp.VM_STRAIN)
\end{lstlisting}

A complete list of parameters for the \code{plotDispTimeHistory} and \code{plotStrainTimeHistory} functions are summarized below.
...
# smoothOrder (int) : the order of the smoothing polynomial
#                     (Default is 2 -> quadratic smoothing)
# interpolate (bool): flag to interpolate values at the specified 
#                     points
#                     (Default is False -> Use nearest subset value)
# ------------------------------------------------------------------
\end{lstlisting}

If the \code{interpolate} flag is set to \code{False}, the nearest subset point to the specified point will be used to create the time history plots.  If this flag is set to \code{True}, the data will be interpolated to the specified point.

As for the line cut plots, there is an additional optional parameter \code{dispComp} to identify the displacement component to plot when calling \code{plotDispTimeHistory}.  Available \code{dispComp} values are \code{X\_DISP}, \code{Y\_DISP}, and \code{DISP\_MAG}.
...
For \code{plotStrainTimeHistory} the corresponding parameter to identify the strain component to plot is \code{strainComp} and available \code{strainComp} values are  \code{X\_STRAIN}, \code{Y\_STRAIN}, \code{SHEAR\_STRAIN}, and \code{VM\_STRAIN}.
...
\subsection{Accessing Raw Time History Data}

For custom analysis or export, the displacement and strain time history data can be retrieved directly as \code{NumPy} arrays using the \code{getDispTimeHistory} and \code{getStrainTimeHistory} functions.  The following listing retrieves the displacement and strain time history.

\begin{lstlisting}
import sundic.post_process as sdpp

# Setup some points to track
points = [(300,300), (800,300), (900,300)]

# Get the displacement time  history data for these points
imgPairs, dispHist, usedPnts = sdpp.getDispTimeHistory('results.sdic', 
                points, dispComp=sdpp.DispComp.X_DISP)

# Get the strain time  history data for these points
imgPairs, strnHist, usedPnts = sdpp.getStrainTimeHistory(
                'results.sdic', points, 
                strainComp=sdpp.StrainComp.VM_STRAIN)
\end{lstlisting}

A complete list of parameters for the \code{getDispTimeHistory}  and \code{getStrainTimeHistory} functions are summarized below.
...
# Returns:
# --------
# imgPairs (ndarray): the image pair indices, shape (nPairs,)
# hist (ndarray)    : the time history values, shape (nPoints, nPairs)
# usedPoints (list) : List of (x, y) points used to sample data
#                     If interpolate=False -> nearest subset
#                     If interpolate=True -> the requested input points
\end{lstlisting}

As for the time history plots, there is an additional optional parameter \code{dispComp} to identify the displacement component to consider when calling \code{getDispTimeHistory}.  Available \code{dispComp} values are \code{X\_DISP}, \code{Y\_DISP}, and \code{DISP\_MAG}.
...
For \code{getStrainTimeHistory} the corresponding parameter to identify the strain component to consider is \code{strainComp} and available \code{strainComp} values are  \code{X\_STRAIN}, \code{Y\_STRAIN}, \code{SHEAR\_STRAIN}, and \code{VM\_STRAIN}.
