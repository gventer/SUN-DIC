...
SUN-DIC has been tested on Windows, macOS, and Linux, with Linux serving as the primary development platform. The examples in this manual assume \textbf{Python \pyversion}. Before proceeding, verify your Python version by running the following command from a terminal or command prompt:
...
\begin{itemize}
	...
	\item On a Mac equipped with an Apple Silicon processor (Mac M1/M2/M3), using the standard Anaconda distribution may cause version conflicts or C++ compilation errors (eg, with \code{llvmlite} or \code{ray}). This is typically due to Anaconda defaulting to x86\_64 emulation.  To ensure a seamless, native ARM64 installation without needing to modify the \code{requirements.txt} file, it is recommended to use \href{https://github.com/conda-forge/miniforge}{Miniforge} instead of Anaconda. 
	\textbf{Setup instructions:}
	\begin{enumerate}
		\item Install the macOS arm64 version of Miniforge.
		\item Open a new terminal to ensure Miniforge is active (you should see the base environment).
		\item Proceed with the standard installation using \code{conda} as described below.
	\end{enumerate}
\end{itemize}
...
\begin{lstlisting}[language=bash]
conda create -n sundic python=(*@\pyversion@*)
...
\begin{lstlisting}[language=bash]
python(*@\pyversion@*) -m venv sundic
\end{lstlisting}
