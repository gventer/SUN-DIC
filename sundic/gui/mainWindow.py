...
from PyQt6.QtGui import QAction, QFont, QFontDatabase, QIcon, QPixmap
...
class UIMainWindow:
    ...
    def setupMainUI(self, parent):
        ...
        self.settingsBut = LeftIconButton(centralwidget, stylesheet=buttonStyle)
        ...
        self.settingsBut.setIcon(QIcon(os.path.join(iconPath, "settings.png")))
        ...
        self.imageSetBut = LeftIconButton(centralwidget, stylesheet=buttonStyle)
        ...
        self.imageSetBut.setIcon(QIcon(os.path.join(iconPath, "imageSet.png")))
        ...
        self.roiBut = LeftIconButton(centralwidget, stylesheet=buttonStyle)
        ...
        self.roiBut.setIcon(QIcon(os.path.join(iconPath, "roi.png")))
        ...
        self.analysisBut = LeftIconButton(centralwidget, stylesheet=buttonStyle)
        ...
        self.analysisBut.setIcon(QIcon(os.path.join(iconPath, "analysis.png")))
        ...
        self.resultsBut = LeftIconButton(centralwidget, stylesheet=buttonStyle)
        ...
        self.resultsBut.setIcon(QIcon(os.path.join(iconPath, "results.png")))
    ...
    def openAction(self):
        ...
        filePath, _ = QFileDialog.getOpenFileName(
            ...
            "Open File",
            "",
            "SUN-DIC Binary Files (*.sdic);;All Files (*)",
            options=options,
        )
        ...
        if filePath:
            ...
            self.parent.settings.ImageFolder = (
                self.parent.getAbsPathRelativeToResultsFile(
        ...
        if self.parent.savePath is not None and os.path.isfile(self.parent.savePath):
            ...
            if resultsFile.containsResults():
                warn = QMessageBox.question(
                    self.parent,
                    "Warning",
                    "Results already exist for this file. If you choose to save the new settings, the previous results will be deleted. Do you want to proceed?",
                    QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                    QMessageBox.StandardButton.No,
                )
                if warn == QMessageBox.StandardButton.No:
    ...
    def saveAsAction(self):
        ...
        fileName, _ = QFileDialog.getSaveFileName(
            self.parent, "Save File", "", "SUNDIC Binary File(*.sdic)"
        )
        ...
        if fileName:
            ...
            if self.parent.savePath is not None and os.path.isfile(
                self.parent.savePath
            ):
                oldDataFile = dataFile.DataFile.openReader(self.parent.savePath)
    ...
    def importSettingsAction(self):
        ...
        try:
            ...
            if hasattr(newSettings, "ImageFolder") and not os.path.isabs(
                newSettings.ImageFolder
            ):
...
class mainProgram(QMainWindow, UIMainWindow):
    ...
    def updateWindowTitle(self):
        ...
        # File already attached so display the name
            ...
            if self.savedFlag:
                self.setWindowTitle(str(os.path.basename(self.savePath)) + " - SUN-DIC")
            else:
                self.setWindowTitle(
    ...
    def getAbsPathRelativeToResultsFile(self, resultsFile, pathValue):
        ...
        if os.path.isabs(resultsFile):
            return os.path.abspath(
                os.path.join(os.path.dirname(resultsFile), pathValue)
            )
...
def main():
    ...
    splash = QSplashScreen(QPixmap(iconsPath), Qt.WindowType.WindowStaysOnTopHint)
    splash.show()
